...
`;
   ...
   */
  async correctGender(originalName: string, correctedGender: Gender): Promise<GenderDetectionResult> {
  ...
  private validateGender(gender: any): Gender {
  ...
  private getPronouns(gender: Gender): string {
    switch (gender) {
      case 'male':
        return 'he/him';
      case 'female':
