import { MediaFile, ProcessedMedia, MediaValidationRule, MEDIA_VALIDATION_RULES } from '../types/agent.types';
...
export class MediaService {
   ...
   */
  validateFile(file: MediaFile): { isValid: boolean; error?: string; category?: string } {
    // Check file size
    if (file.size === 0) {
   ...
   */
  async processFile(file: MediaFile): Promise<ProcessedMedia> {
    // Validate file first
   ...
   */
  async attachMediaToSession(sessionId: string, mediaFiles: ProcessedMedia[]): Promise<void> {
    try {
      ...
      await this.complaintsService.addEvidenceToComplaint(
        session.complaintId,
   ...
   */
  private detectFileCategory(mimeType: string): string | null {
   ...
   */
  private async uploadFile(file: MediaFile): Promise<{ url: string; key: string }> {
   ...
   */
  private async getSessionBySessionId(sessionId: string): Promise<any> {
  ...
  getSupportedFileTypes(): string {
    const types = Object.entries(MEDIA_VALIDATION_RULES)
      .map(([category, rules]) => {
        const maxSizeMB = Math.round(rules.maxSize / (1024 * 1024));
   ...
   */
  async processBatchFiles(files: MediaFile[]): Promise<ProcessedMedia[]> {
